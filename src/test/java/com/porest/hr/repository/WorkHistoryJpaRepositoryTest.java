...
import com.porest.hr.work.repository.WorkHistoryJpaRepository;
...
@DataJpaTest
@Import({WorkHistoryJpaRepository.class, TestQuerydslConfig.class})
@Transactional
@DisplayName("JPA 업무이력 레포지토리 테스트")
class WorkHistoryJpaRepositoryTest {
    @Autowired
    private WorkHistoryJpaRepository workHistoryRepository;
    ...
    void save() {
        ...
        // then
        Optional<WorkHistory> findHistory = workHistoryRepository.findByRowId(workHistory.getRowId());
    ...
    void findByIdEmpty() {
        // when
        Optional<WorkHistory> findHistory = workHistoryRepository.findByRowId(999L);
    ...
    void findAllWithStartDateOnly() {
        ...
        // when
        WorkHistorySearchCondition condition = new WorkHistorySearchCondition();
        condition.setStartDate(LocalDate.of(2025, 2, 1));
        List<WorkHistory> histories = workHistoryRepository.findAll(condition);
    ...
    void delete() {
        ...
        // when
        WorkHistory foundHistory = workHistoryRepository.findByRowId(workHistory.getRowId()).orElseThrow();
    ...
    void updateWorkHistory() {
        ...
        // then
        WorkHistory updatedHistory = workHistoryRepository.findByRowId(workHistory.getRowId()).orElseThrow();
    ...
    @Test
    @DisplayName("오래된 순으로 정렬하여 조회")
    void findAllSortByOldest() {
        // given
        workHistoryRepository.save(WorkHistory.createWorkHistory(
                LocalDate.of(2025, 1, 3), user, group, part, division,
                new BigDecimal("8.0"), "세번째"
        ));
        workHistoryRepository.save(WorkHistory.createWorkHistory(
                LocalDate.of(2025, 1, 1), user, group, part, division,
                new BigDecimal("8.0"), "첫번째"
        ));
        workHistoryRepository.save(WorkHistory.createWorkHistory(
                LocalDate.of(2025, 1, 2), user, group, part, division,
                new BigDecimal("8.0"), "두번째"
        ));
        em.flush();
        em.clear();
        ...
        assertThat(histories.get(0).getContent()).isEqualTo("첫번째");
        assertThat(histories.get(1).getContent()).isEqualTo("두번째");
        assertThat(histories.get(2).getContent()).isEqualTo("세번째");
    }

    @Test
    @DisplayName("그룹 Seq로 필터링하여 조회")
    void findAllWithGroupSeq() {
        // given
        ...
        // when
        WorkHistorySearchCondition condition = new WorkHistorySearchCondition();
        condition.setGroupSeq(group.getRowId());
        List<WorkHistory> histories = workHistoryRepository.findAll(condition);
    ...
    void findAllWithUserName() {
        ...
        // when
        WorkHistorySearchCondition condition = new WorkHistorySearchCondition();
        condition.setUserName("홍길동");
        List<WorkHistory> histories = workHistoryRepository.findAll(condition);

        // then
        assertThat(histories).hasSize(1);
        assertThat(histories.get(0).getContent()).isEqualTo("홍길동 작업");
    }

    @Test
    @DisplayName("findAllStream으로 스트림 조회")
    void findAllStream() {
        // given
        workHistoryRepository.save(WorkHistory.createWorkHistory(
                LocalDate.of(2025, 1, 1), user, group, part, division,
        ...
        WorkHistorySearchCondition condition = new WorkHistorySearchCondition();
        condition.setUserId("user1");
        long count = workHistoryRepository.findAllStream(condition).count();
    ...
    void findAllWithPartSeq() {
        ...
        // when
        WorkHistorySearchCondition condition = new WorkHistorySearchCondition();
        condition.setPartSeq(part.getRowId());
        List<WorkHistory> histories = workHistoryRepository.findAll(condition);
    ...
    void findAllWithDivisionSeq() {
        ...
        // when
        WorkHistorySearchCondition condition = new WorkHistorySearchCondition();
        condition.setDivisionSeq(division.getRowId());
        List<WorkHistory> histories = workHistoryRepository.findAll(condition);
